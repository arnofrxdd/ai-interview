 ...
 * - Calls `response.create` to trigger speech ONLY when resuming from a "Clutch" pause.
 */
...
6. When SYSTEM DATA or SYSTEM DIRECTIVES are injected, seamlessly adapt your next turn. Do not abruptly interrupt yourself or double-ask. Simply incorporate the new guidance naturally the NEXT time you speak.
`;
    }, []);
...
Task: Return a JSON object strictly matching this schema based on the LAST 1-2 messages:
{
  ...
  "is_substantive_answer": boolean, // TRUE ONLY IF the candidate provided a meaningful, complete answer to an interview question. EXPLICITLY FALSE if they are just confirming details, saying "yes/no/got it", agreeing to move on, asking to start/continue, or talking about the interview process itself.
...
Return ONLY valid JSON. No markdown formatting.`;
            ...
            if (analysis.suggested_phase_advance !== 'none' && analysis.suggested_phase_advance !== phaseRef.current) {
                ...
                if (target === 'warmup' && phaseRef.current === 'greeting') {
                    ...
                    // Use false for forceResponse so we don't trigger double speak if the AI already naturally responded
                    injectSystemMessage(`SYSTEM DIRECTIVE: Transition to warmup naturally. You can use this question: "${firstWQ}"`, false);
                    return; // Skip scoring on transition turn
            ...
            // ONLY if it is a substantive answer. If it's a dodge/refusal or meta-talk, we do nothing and let the Actor handle it natively.
            if (analysis.is_substantive_answer && analysis.answer_summary) {
                if (phaseRef.current === 'warmup') {
                    ...
                    if (newCount < WARMUP_QUESTIONS_REQUIRED) {
                        ...
                        warmupQIndexRef.current += 1;
                        if (nextQ) injectSystemMessage(`SYSTEM DIRECTIVE: Acknowledge their answer briefly, then ask the next warmup question: "${nextQ}"`, false);
                    } else {
                        // Let observer catch phase advance next turn, or force it
                        injectSystemMessage(`SYSTEM DIRECTIVE: Warmup complete. Suggest transitioning to the technical portion of the interview.`, false);
                    }
    ...
    const handleCvLookup = async (topic: string, isPaused: boolean) => {
        ...
        try {
            ...
            // ONLY force a response if the AI had explicitly paused. Otherwise, silently inject.
...
Return JSON only:
{
  ...
  "suggested_followup": "<specific follow-up question referencing what they said, max 25 words>"
            ...
            // Deepen if needed. Use forceResponse=false so it naturally weaves it in next turn
            if (parsed.depth_assessment === 'shallow' && ansCount < 3) {
                ...
                injectSystemMessage(`SYSTEM DIRECTIVE: The candidate's answer was surface-level. Probe deeper with: "${parsed.suggested_followup}"`, false);
            } else {
                ...
                if (parsed.suggested_followup) {
                    injectSystemMessage(`SYSTEM DIRECTIVE: Follow up naturally with: "${parsed.suggested_followup}"`, false);
                }
            }
    ...
    const handleTopicExhausted = async (exhaustedTopic: string) => {
        ...
        try {
            ...
            // Inject the bridge instruction. Use forceResponse=false so the AI seamlessly transitions on its next turn.
            injectSystemMessage(`SYSTEM DIRECTIVE: Topic "${exhaustedTopic}" is complete. Transition smoothly to a new topic: "${nextTopic}". Lead with this question: "${nextQ}"`, false);
    ...
    const handleGapDetection = async () => {
        ...
        try {
            ...
            if (gap && gap !== 'none' && gap.length < 80) {
                ...
                if (gapQ) {
                    injectSystemMessage(`SYSTEM DIRECTIVE: JD Gap Identified - "${gap}". At an appropriate moment soon, probe this by asking: "${gapQ}"`, false);
    ...
    const generateInterviewStrategy = async () => {
        ...
        try {
            ...
            // Push the strategy into the RT context silently (forceResponse = false) so it pivots naturally on its next turn
