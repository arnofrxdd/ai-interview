...
export default function AIInterviewInterface() {
    ...
    const buildSystemPrompt = useCallback((): string => {
        ...
        let stageGuidance = '';
        if (stage === 'warmup') stageGuidance = 'STAGE: WARMUP. Ask personal, non-CV questions to break the ice and understand their motivations.';
...
- Do NOT summarize or rephrase the candidate's answer back to them unless they explicitly ask
- Keep acknowledgments minimal — no praise, no excessive encouragement
...
- Occasional natural fillers are fine: "Right, so...", "Yeah, that makes sense", "Interesting..."  
    ...
    const resetSilenceTimer = useCallback(() => {
        ...
        silenceTimeoutRef.current = setTimeout(() => {
            if (!isEndingRef.current && dcRef.current?.readyState === 'open') {
                const lastQ = currentQRef.current ? ` The last question you asked was: "${currentQRef.current}".` : "";
                sendRtMessage({
                    type: 'response.create',
                    response: {
                        instructions: `They've been quiet for a bit.${lastQ} Check in casually in one sentence, like a real person: acknowledge no rush, ask if they want more time or a rephrase. Do not sound formal or scripted.`,
                    },
                });
            }
            ...
            case 'response.audio_transcript.done': {
                ...
                if (transcript) {
                    ...
                    if (transcript.includes('?')) {
                        ...
                        // Don't call resetSilenceTimer here — tick() will call it once AI audio stops
                    }
                }
        ...
        try {
            ...
            const tick = () => {
                ...
                if (outAnalyser && outData) {
                    outAnalyser.getByteFrequencyData(outData as any);
            ...
            dc.onopen = () => {
                ...
                setTimeout(() => {
                    ...
                    sendRtMessage({
                        type: 'response.create',
                        response: {
                            instructions: `You are starting a job interview. Greet ${candidateName || 'the candidate'} warmly by name. Introduce yourself briefly as Aria, a senior interviewer. Then ask: "Before we get into the structured questions, I'd love to hear something about you that might not come through on paper — what's been genuinely exciting you in your work lately?" Keep it to 2-3 short, natural sentences. Use contractions. Avoid corporate phrasing and avoid long sentences.`,
                        },
                    });
