...
export default function AriaV4() {
    ...
    const [interviewDuration, setInterviewDuration] = useState(10); // in minutes
    ...
    const startSilenceTimer = useCallback((ms?: number) => {
        ...
        silenceIntervalRef.current = setInterval(() => {
            // FREEZE: If AI is actually speaking (audio detection), reset the start time
        ...
        silenceTimerRef.current = setTimeout(() => {
            ...
            if (silencePromptCountRef.current >= 3) {
                const d_tid = addIntelLog('director', 'Max silence limit (3 strikes) reached. Terminating session.');
            ...
            injectSystemMessage(`SYSTEM DIRECTIVE: The user has been completely silent (Strike ${silencePromptCountRef.current}/3). Politely ask if they are still there or if they need more time.`, true);
            // Reset back to normal default for next attempt
    ...
    const buildActorPrompt = useCallback((forPhase?: AppPhase): string => {
        ...
        const phaseInstructions: Record<AppPhase, string> = {
            ...
            greeting: `Greet ${name} warmly and uniquely. You are Aria, a senior interviewer. Use any available context to make the greeting personal (e.g. mention their background or a specific skill if you have current CV info), otherwise be exceptionally welcoming. Ask if they can hear you clearly and are comfortable to begin. Keep it to 2 sentences.`,
            warmup: `Ask purely personal questions to build rapport and understand ${name}'s personality. This phase is about their life, hobbies, and interests outside of work. Be very casual and human (e.g., "Why don't you tell me a bit about yourself, man?"). Avoid all technical or career-related questions here.`,
            ...
            wrapup: `The technical interview is over. Move to final house-keeping: ask if they have questions, mention next steps, and keep it friendly but concise. No more technical questions.`,
            closing: `Thank ${name} warmly. Tell them the interview is complete. Give one sentence on next steps. Wish them well and stop.`,
            report: 'Silent.',
        };
        const isInterviewPhase = p === 'interview' || p === 'wrapup' || p === 'closing';
...
2. PERSONALITY: NEVER summarize the candidate's last answer or give excessive praise (e.g., "That's a great answer!", "I love how you explained that"). Be professional, direct, and conversational without being sycophantic.
3. IMPORTANT: You have a backend assistant retrieving CV data and documents for you in real-time. NEVER tell the user "I don't have access to your CV" or "I am an AI." You DO have access, it just takes a second to load.
4. THE CLUTCH: If you are asked to verify, check, or summarize the candidate's CV/resume, OR asked a specific background question not in your context:
${isInterviewPhase ?
                `   - SCRIPT: Acknowledge conversationally (e.g., "Yeah, I'll definitely check your CV for that..."). 
   - ACTION: DO NOT STOP SPEAKING. Instead, continue with a bridge or technical follow-up. The data will be available for our next turn.` :
                `   - SCRIPT: Say EXACTLY: "Let me pull up your file...", "Give me a second to check your CV...", or "Let me look at my notes..." 
   - ACTION: You MUST STOP SPEAKING IMMEDIATELY. The backend will instantly inject the correct SYSTEM DATA and trigger our next turn.`
            }
5. Do NOT hallucinate facts. Do NOT apologize. 
6. STRICT ONE QUESTION RULE: You MUST NEVER ask more than one question per response. If you ask a question, you must STOP speaking immediately. Do not stack questions.
7. EPHEMERAL DIRECTIVES: When you receive a "SYSTEM DIRECTIVE", it applies ONLY to your very next turn. Follow it strictly, ask exactly one question if prompted to, and then consider it obsolete. ALWAYS prioritize the most recent directive over past ones.
`;
    }, []);
    ...
    const transitionPhase = useCallback((newPhase: AppPhase) => {
        ...
        if (newPhase === 'closing') {
            // STOP: Removed automatic mic disabling to allow final questions/farewell from user.
        }
    ...
    const startInterviewTimer = useCallback(() => {
        ...
        interviewTimerRef.current = setInterval(() => {
            ...
            // Smart Freezing: Only increment elapsed time if the AI is NOT speaking (Real Audio + Status)
            ...
            if (elapsed >= totalMs) {
                ...
                // SOFT PIVOT: No forceResponse, transition to wrapup
    ...
    const runObserverPipeline = useCallback(async () => {
        ...
        try {
            ...
            let prompt = '';
            let systemRole = 'You are a highly perceptive primary supervisor. Your role is to monitor the interview transcript critically. You must identify candidate intent, state transitions, AND supervise the AI (Aria) for errors, rambling, or tone issues.';
...
{
  "is_filler_pause": boolean, 
  "is_substantive_answer": boolean, // TRUE if candidate provided ANY answer to a warmup question.
  "answer_summary": "string", // Summarize the answer in 20 words.
  "suggested_phase_advance": "none" | "warmup" | "interview", // Suggest "interview" if Warmup Qs >= ${WARMUP_QUESTIONS_REQUIRED} OR if candidate expresses a clear desire to start the interview.
  "is_off_topic": boolean, // TRUE if candidate is being extremely unhelpful (one-word answers like "no", "nah", "yeah") OR if AI and candidate are cycling on meta-talk/feelings.
  "should_force_pivot": boolean, // TRUE if you've been off-topic for 2+ messages.
  "ai_rambling": boolean, // TRUE if the AI's last message was longer than 40 words OR repeated previous points unnecessarily.
  "ai_hallucination_or_tone_issue": "string", // If the AI apologized, said "I am an AI", summarized excessively, or gave over-the-top praise (e.g., "That's a fantastic answer!"). 
  "candidate_struggling": boolean,
  "red_flag_detected": "string", // Significant candidate behavioral issues (offensive language, extreme rudeness). NOT for connection/silence.
  "requested_pause_seconds": number
} # IMPORTANT: DO NOT suggest "interview" early based on single-word answers like "and", "yes", "no". Only suggest early graduation if the candidate is PERSISTENTLY unhelpful over multiple turns (>=2) OR explicitly asks to "move on" or "start the technical part".`;
            }
            // ── PHASE: INTERVIEW ──
...
Task: Return a JSON object matching this schema:
{
  ...
  "answer_summary": "string", // Summarize the answer in 20 words.
  "cv_topic": "string", 
  "topic_exhausted": boolean, // TRUE if topic is covered OR if candidate is stuck/stalling on it.
  "suggested_phase_advance": "none" | "wrapup", // Suggest "wrapup" if Scored Answers >= ${numQuestionsRef.current} OR if candidate is persistently stalling/struggling across multiple questions.
  "candidate_ready_to_end": boolean, // TRUE ONLY if the candidate explicitly says they want to stop the interview now.
  "is_off_topic": boolean, // TRUE if you've been on a meta-talk cycle for 2+ messages.
  "should_force_pivot": boolean, 
  "ai_rambling": boolean, // TRUE if the AI is repeating itself, summarizing the candidate's last answer in 10+ words, or using excessive fluff.
  "ai_hallucination_or_tone_issue": "string", // Identify if the AI provided incorrect technical info, apologized, mentioned being an AI, or was sycophantic.
  "candidate_struggling": boolean,
  "red_flag_detected": "string", // Significant candidate behavioral issues ONLY. NOT for technical/connection issues.
  "callback_opportunity": "string",
  "requested_pause_seconds": number,
  "is_complex_question": boolean
}`;
...
Task: Return a JSON object matching this schema:
{
  ...
  "candidate_has_final_question": boolean, 
  "candidate_ready_to_end": boolean, 
  "should_end_call": boolean, // TRUE ONLY if BOTH sides have acknowledged the end. 
  "answer_summary": "string", 
  "suggested_phase_advance": "none" | "closing" | "report", // Suggest "report" immediately if the candidate has said "goodbye", "thanks", or acknowledged completion.
  "is_off_topic": boolean,
  "should_force_pivot": boolean,
  "ai_rambling": boolean, // TRUE if AI is stalling or using repetitive farewells.
  "ai_hallucination_or_tone_issue": "string" // Identify any sycophancy or tone errors.
}`;
            }
            ...
            const analysis: ObserverAnalysis = {
                ...
                candidate_ready_to_end: false,
                should_end_call: false,
                is_off_topic: false,
                should_force_pivot: false,
                ...parsedJson
            };
            ...
            // Fix 1: Replace the broken wrapup counter check
            ...
            } else if (analysis.is_substantive_answer || analysis.is_complex_question) {
                driftCountRef.current = 0; // Reset on productive turns
            }
            ...
            if (analysis.ai_rambling) {
                setObserverActivity('injecting');
                addIntelLog('director', 'AI rambling detected. Limiting word count.');
            ...
            if (analysis.ai_hallucination_or_tone_issue) {
                setObserverActivity('injecting');
                addIntelLog('director', `Tone/Hallucination correction: ${analysis.ai_hallucination_or_tone_issue}`);
            ...
            if (analysis.is_off_topic && !shouldForcePivot && !injectedDirector) {
                setObserverActivity('injecting');
                const d_tid = addIntelLog('director', 'Drift detected (Turn 1). Sending refocus warning.');
                injectSystemMessage("SYSTEM DIRECTIVE: You are drifting off-topic. Acknowledge the user VERY briefly and pull the conversation back to the current phase goals immediately.", false);
                updateIntelLog(d_tid, 'done', 'Refocus warning injected ✓');
                injectedDirector = true;
            }

            if (shouldForcePivot && !injectedDirector) {
                setObserverActivity('injecting');
                const d_tid = addIntelLog('director', 'Persistent drift (Turn 2). Forcing hard refocus...');
                let pivotMsg = "CRITICAL: Stop the current tangent. Immediately move back to the core interview goals.";
                if (currentPhase === 'warmup') {
                    const nextQ = warmupQueueRef.current[warmupQIndexRef.current] || "Why don't you tell me a bit about yourself, man? What makes you tick outside of work?";
                    pivotMsg = `CRITICAL: Halt the current topic. Acknowledge and immediately ask ONLY this next warmup question: "${nextQ}"`;
                    warmupQIndexRef.current += 1;
                }
                if (currentPhase === 'interview') pivotMsg = "CRITICAL: The conversation is off-topic. Immediately pivot back to the next technical topic or follow-up question.";
                if (currentPhase === 'wrapup') pivotMsg = "CRITICAL: Wrap-up is stalling. Immediately address the final non-technical housekeeping details or exit the call.";

                injectSystemMessage(`CRITICAL RE-FOCUS: ${pivotMsg}`, true);
                driftCountRef.current = 0; // Reset after forcing
                setDriftCount(0);
            ...
            if (analysis.needs_cv_lookup && analysis.cv_topic && currentPhase !== 'warmup' && currentPhase !== 'greeting') {
                handleCvLookup(analysis.cv_topic, analysis.is_filler_pause, currentPhase);
            } else if (analysis.is_filler_pause) {
                // Only force if we are genuinely stuck in a "Clutch" filler pause
            ...
            // ── 5. Phase Advances (Priority Logic) ──
            let target = analysis.suggested_phase_advance;

            // If Observer wants to move to Technical, we MUST bypass the Warmup question loop.
            // PATIENCE GUARD: Don't auto-graduate on "engagement" until at least 2 warmup turns.
            // But ALWAYS graduate if the candidate explicitly asked to skip.
            const isExplicitSkip = analysis.answer_summary?.toLowerCase().includes('skip') ||
                analysis.answer_summary?.toLowerCase().includes('start interview');

            if (target === 'interview' && (phaseRef.current === 'warmup' || phaseRef.current === 'greeting')) {
                if (warmupCountRef.current >= 2 || isExplicitSkip) {
                    const t_tid = addIntelLog('director', 'Candidate engagement suggests early technical pivot...');
                    setQuestionCount(0);
                    transitionPhase('interview');
                    startInterviewTimer();
                    generateInterviewStrategy();
                    ...
                    // Stay in warmup for now!
                    target = 'none';
                }
            }

            // Fix 2: Add guard so closing only triggers after user actually responded
            const userRespondedInWrapup = wrapupCountRef.current >= 1;

            if (target === 'closing' && !userRespondedInWrapup) {
                target = 'none'; // Don't close until user has spoken at least once in wrapup
            }

            // AUTO-ADVANCE: If we hit the limit, FORCE wrapup even if Observer is hesitant
            if (phaseRef.current === 'interview' && scoresRef.current.length >= numQuestionsRef.current) {
                target = 'wrapup';
            }

            // Fix 3: Transition to closing ONLY if 3 turns completed OR candidate explicitly ready
            if (phaseRef.current === 'wrapup') {
                if (!userRespondedInWrapup) {
                    target = 'none';
                } else if (analysis.candidate_ready_to_end || wrapupCountRef.current >= 3) {
                    target = 'closing';
                } else {
                    target = 'none';
                }
            ...
            if (analysis.should_end_call) {
                ...
                if (isAISpeakingRef.current) {
                    ...
                    updateIntelLog(d_tid, 'done', 'Intent: Natural Conclusion (Waiting for audio) ✓');
                } else {
            ...
            if (target !== 'none' && target !== (phaseRef.current as any)) {
                if (target === 'report') {
                    // STOP: Removed immediate endCall(). 
                    // We will let the user (or a manual trigger) end the session when they are truly done.
                    return;
                }

                if (target === 'wrapup') {
                    const d_tid = addIntelLog('director', 'Technical phase complete. Transitioning to wrap-up...');
                    // SOFT INJECTION: No immediate response trigger.
                    injectSystemMessage("SYSTEM DIRECTIVE: Technical portion is NOW finished. Transition to the wrap-up phase. Strictly NO technical or career questions from here on. Ask if they have any final questions for you or mention next steps.", false);
                    transitionPhase('wrapup');
                ...
                if (target === 'closing') {
                    ...
                    // SOFT INJECTION: No immediate response trigger. AI will wait for user's turn.
                    injectSystemMessage("SYSTEM DIRECTIVE: Wrap-up complete. Provide a final, warm farewell and stop speaking. Do not ask any more questions.", false);
                ...
                if (target === 'warmup' && phaseRef.current === 'greeting') {
                    ...
                    const firstWQ = warmupQueueRef.current[0] || "Before we dive in, why don't you tell me a bit about yourself, man? What do you love doing when you're not at a desk?";
                    warmupQIndexRef.current = 1;
                    topicStartTimeRef.current = Date.now();
    ...
    const handleCvLookup = async (topic: string, isPaused: boolean, currentPhase: AppPhase) => {
        // 1. Check Memory (Cache) to "save memory" for future
        if (cvLookupCacheRef.current.has(topic)) {
            ...
            // Logic Gating: 
            // - If AI used a filler pause (isPaused), we MUST force response to un-stick it.
            // - If Warmup phase AND the AI isn't currently speaking, we force response to keep momentum.
            // - Otherwise (Interview phase), we DO NOT force (soft injection) to avoid double-asking.
            const shouldForce = isPaused || (currentPhase === 'warmup' && callStatus !== 'Speaking...');

            injectSystemMessage(`SYSTEM DATA - CV LOOKUP [MEMORY] for "${topic}": ${cachedResult}. Use this information seamlessly to reply to the candidate.`, shouldForce);
            return;
        }
        ...
        try {
            ...
            injectSystemMessage(`SYSTEM DATA - CV LOOKUP for "${topic}": ${result}. Use this information seamlessly to reply to the candidate.`, shouldForce);
            updateIntelLog(tid, 'done', `CV lookup: "${topic}" ✓`);
        } catch {
            const shouldForce = isPaused || (currentPhase === 'warmup' && callStatus !== 'Speaking...');
            injectSystemMessage('SYSTEM DIRECTIVE: Resume conversation naturally. Acknowledge the user but transition smoothly to the next question.', shouldForce);
                ...
                if (finalScore <= 4) {
                    ...
                    if (parsedCvDeepDive.cv_followup) {
                        ...
                        if (topicInfo) {
                            ...
                            injectSystemMessage(`SYSTEM CONTEXT: The candidate's CV mentions work on "${parsedCvDeepDive.project_reference}". A potential deep-dive for your NEXT turn: "${parsedCvDeepDive.cv_followup}"`, false);
                        }
                    }
                }
            } else {
...
JD: ${jdTextRef.current.slice(0, 1000)}

Task: Generate a technical interview strategy. 
RULES:
1. STRICTLY FORBID generic "tell me about a project" or "what is your experience" questions.
2. EVERY question MUST include a specific technical keyword, architecture pattern, or tool from the CV/JD.
3. LVL 1 (Surface): Foundational concept within a topic.
4. LVL 2 (Technical): Implementation detail or trade-off.
5. LVL 3 (Deep): Architecture, edge cases, or deep problem-solving.

Return JSON strictly matching:
{
  "topics": ["<topic1>", "<topic2>", "<topic3>", "<topic4>", "<topic5>"],
  "questionQueue": {
    "<topic1>": ["<LVL1 Specific Q>", "<LVL2 Specific Q>", "<LVL3 Specific Q>"],
    ...
  },
  "gapAreas": ["<gap>"]
}`,
                'Technical Strategy Engine. Factual and direct.',
                usageRef,
                true
            );
            ...
            // PIVOT DIRECTIVE: SOFT INJECTION
            // The Actor already acknowledged the transition, so we just prep the first technical question for its NEXT turn.
            injectSystemMessage(`SYSTEM DIRECTIVE: WARMUP SKIP DETECTED / TECHNICAL PHASE START. Your NEXT turn MUST use this technical question: "${firstQ}". Do not ask generic rapport questions.`, false);
    ...
    const generateWarmupQuestions = async () => {
        try {
            const raw = await callMini(
                `CV Summary: ${cvSummaryRef.current.slice(0, 500)}\n\nGenerate 3 unique, engaging warmup questions about the candidate's personality and personal life. 
        RULES:
        1. FOCUS ONLY on personal life, hobbies, passions, or interesting off-work experiences.
        2. COMPLETELY AVOID technical or professional questions (no "journey into the field" or "role" questions).
        3. Be super casual and friendly (e.g., using "man", "buddy", or "anyway" if appropriate).
        4. Keep them brief.
        Return JSON array only: ["<q1>", "<q2>", "<q3>"]`,
                'Unique personal warmup generator.', usageRef, true
            );
            let qs: string[] = [];
    ...
    const handleRtEvent = useCallback((ev: Record<string, unknown>) => {
        switch (ev.type as string) {
            ...
            case 'input_audio_buffer.speech_started':
                ...
                silencePromptCountRef.current = 0; // Reset silence strike counter
            ...
            case 'response.done': {
                ...
                // STOP: Removed automatic endCall() timeout. 
                // Let the candidate speak/finish before manually ending the session.
            ...
            case 'response.output_item.added': {
                ...
                if ((item?.role as string) === 'assistant') {
                    ...
                    // If the AI starts speaking, we stop the observer to prevent mid-turn interruptions
                    isObserverRunningRef.current = false;
                    setIsObserverActive(false);
                    setObserverActivity('idle');
                }
    ...
    const startCall = useCallback(async () => {
        ...
        setInterviewTimeLeft(interviewDuration * 60); setActiveRtItems([]);
        ...
        try {
            ...
            pc.ontrack = e => {
                ...
                // ── Real-time AI Audio Detection (Sound-based) ──
                ...
                const checkAIAudio = () => {
                    ...
                    if (avg > 2) {
                        // AI is actually detected via frequency analysis
                        ...
                        // Audio drop-off — apply grace period before considering it silent
                        if (!aiSilenceTimerRef.current) {
                            aiSilenceTimerRef.current = setTimeout(() => {
                                isAISpeakingRef.current = false;
                                aiSilenceTimerRef.current = null;

                                // If we were waiting for the AI to finish speaking to end the call, do it now!
                                if (pendingEndRef.current) {
                                    endCall();
                                }
                            }, 600); // 600ms grace period for natural speech gaps
                        }
                    }
                ...
                setTimeout(() => {
                    ...
                    greetingTimeoutRef.current = setTimeout(() => {
                        if (phaseRef.current === 'greeting' && !isEndingRef.current) {
                            transitionPhase('warmup');
                            // Only force if AI isn't already talking
                            const shouldForce = callStatus !== 'Speaking...';
                            injectSystemMessage(`SYSTEM DIRECTIVE: Transition to warmup naturally. First question: "${warmupQueueRef.current[0] || 'How are you today?'}"`, shouldForce);
                        }
                    ...
                    <div className="card">
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                            {[2, 5, 10, 15].map(m => (
                                <button
                                    key={m}
                                    onClick={() => setInterviewDuration(m)}
                                    style={{
                                        flex: 1,
                                        padding: '8px 0',
                                        borderRadius: 8,
        ...
        <>
            ...
            <div className="live">
                ...
                <div className="live-left">
                            ...
                            <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ fontSize: 11, fontWeight: 600 }}>
                                        {phaseRef.current === 'interview' ? 'Interview Observer' : phaseRef.current === 'closing' ? 'Closing Observer' : 'Warmup Observer'}
                                    </div>
                                    {isObserverActive && (
                                ...
                                <div style={{ fontSize: 9, color: 'var(--text3)' }}>Phase-specific intent analysis</div>
                            </div>
                        </div>
                    </div>
                    ...
                    {lastObserverAnalysis && (
                        <div className="intel-feed" style={{ borderColor: 'var(--violet)44', background: 'var(--violet)05' }}>
                            ...
                            <div style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span style={{ fontSize: 9, color: 'var(--text3)' }}>FILLER / STALLING:</span>
                                    <span style={{ fontSize: 9, color: lastObserverAnalysis.is_filler_pause ? 'var(--amber)' : 'var(--text3)' }}>
                                        {lastObserverAnalysis.is_filler_pause ? 'YES' : 'NO'}
                                    </span>
                                </div>
                    ...
                    <div className="intel-feed" style={{ borderColor: 'var(--line)66', background: 'var(--bg3)44', marginTop: 12 }}>
                        <div className="intel-hd" style={{ borderBottomColor: 'var(--line)44' }}>
                            <span style={{ fontSize: 9, color: 'var(--text3)', fontWeight: 700 }}>SYSTEM MONITORING</span>
                        </div>
                        <div style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <span style={{ fontSize: 9, color: 'var(--text3)' }}>DRIFT TURNS:</span>
                                <span style={{ fontSize: 9, color: driftCount >= 2 ? 'var(--red)' : 'var(--text2)', fontWeight: 800 }}>{driftCount} / 2</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <span style={{ fontSize: 9, color: 'var(--text3)' }}>WARMUP TURNS:</span>
                                <span style={{ fontSize: 9, color: 'var(--text2)' }}>{warmupCount} / {WARMUP_QUESTIONS_REQUIRED}</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <span style={{ fontSize: 9, color: 'var(--text3)' }}>WRAPUP TURNS:</span>
                                <span style={{ fontSize: 9, color: 'var(--text2)' }}>{wrapupCountRef.current} / 3</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <span style={{ fontSize: 9, color: 'var(--text3)' }}>SILENCE STRIKES:</span>
                                <span style={{ fontSize: 9, color: 'var(--red)' }}>{silenceStrikes} / 3</span>
                            </div>
                        </div>
                    </div>
