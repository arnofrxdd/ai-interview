...
{
  "is_filler_pause": boolean, // Did the AI literally just say "Let me check...", "Pulling up your file...", or explicitly pause to look something up?
  ...
  "needs_cv_lookup": boolean, // TRUE if the user asks to verify, check, or review their CV, OR asks a specific background question.
  "cv_topic": "string", // The topic to look up. If the request is generic (e.g., "check my CV details"), output "General Background Summary".
    ...
    const handleCvLookup = async (topic: string, isPaused: boolean) => {
        ...
        try {
            const result = await callMini(
                `FULL CV:\n${cvTextRef.current}\n\nExtract ONLY info about: "${topic}". 2-3 sentences max. If not found, say "Not mentioned in CV."`,
                'CV lookup tool. Factual, brief, no commentary.',
                usageRef
            );

            // Inject the found data. If the AI was paused ("Let me check..."), force it to speak immediately.
            injectSystemMessage(`SYSTEM DATA - CV LOOKUP for "${topic}": ${result}. Use this information seamlessly.`, isPaused);
            updateIntelLog(tid, 'done', `CV lookup: "${topic}" ✓`);
        } catch {
            injectSystemMessage('SYSTEM DIRECTIVE: Resume conversation naturally. Do not explicitly state the lookup failed, just pivot.', isPaused);
