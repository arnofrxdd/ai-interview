...
- Do NOT start asking deep technical questions yet.`;
    ...
    if (phase === 'interview') {
        const activeTopic = topics.find(t => t.status === 'active');
        const coveredTopics = topics.filter(t => t.status === 'covered' || t.status === 'exhausted' || t.status === 'failed');
        const pendingTopics = topics.filter(t => t.status === 'pending');

        const roadmapLines = [
            activeTopic ? `→ CURRENT FOCUS: ${activeTopic.name} (${activeTopic.source.toUpperCase()}) — ${activeTopic.turnCount}/${MAX_TOPIC_TURNS} turns` : '→ MISSION: Transition to Wrap-up',
            '',
            'STORYLINE PROGRESS:',
            ...coveredTopics.map((t, i) => `  [DONE] Topic ${i + 1}: ${t.name} (${t.status.toUpperCase()})`),
            ...pendingTopics.map((t, i) => `  [NEXT] Topic ${coveredTopics.length + 2 + i}: ${t.name}`),
        ].join('\n');
...
[STRICT TOPIC RULE]: 
- You MUST only ask questions related to the CURRENT FOCUS mentioned above. 
- NEVER ask about [DONE] topics.
...
export default function AriaV6() {
    ...
    const activeDirectiveRef = useRef<string>('');
    ...
    const updateAgentInstructions = useCallback(() => {
        ...
        const fullPrompt = buildRTSystemPrompt({
            ...
            briefText: activeDirectiveRef.current || b,
        });

        // We use the attribute 'instructions' to send the full persona + brief
        const syncId = Date.now().toString();
        console.log(`[LiveKit Sync] Pushing update ${syncId} (${phaseRef.current})...`);
        setCurrentInstructions(fullPrompt);
        lkRoom?.localParticipant.setAttributes({
            instructions: fullPrompt,
            sync_id: syncId
        });
    }, [lkRoom]);

    const syncInstructionsRef = useRef(updateAgentInstructions);
    useEffect(() => {
        syncInstructionsRef.current = updateAgentInstructions;
    }, [updateAgentInstructions]);

    // ── Inject Brief to RT (Now using LiveKit Sync) ──
    const injectBrief = useCallback((briefText: string, _forceResponse = false) => {
        if (isEndingRef.current) return;
        activeDirectiveRef.current = briefText;
        setCurrentBrief(briefText);
        updateAgentInstructions();
        // Clear directive after substantial time to allow agent to act
        setTimeout(() => { activeDirectiveRef.current = ''; }, 6000);
    }, [updateAgentInstructions]);

    // ── Full Session Update (Now using LiveKit Sync) ──
    const updateRTSession = useCallback((_phase: AppPhase, _topics: TopicEntry[], _extraBriefParams?: any) => {
        updateAgentInstructions();
    }, [updateAgentInstructions]);
...
Return ONLY JSON:
{
  ...
  "answer_summary": "15 word technical summary"
            ...
            if (activeIdx !== -1) {
                ...
                if (coverage.shouldAdvance) {
                    ...
                    if (coverage.partial && coverage.forceAdvance) {
                        directive = `\n\n[IMMEDIATE TOPIC SHIFT]: Candidate was evasive. END the current topic on ${updatedTopics[activeIdx].name} NOW. Pivot to: ${nextTopicName}.`;
                    } else if (coverage.exhausted) {
                        directive = `\n\n[CRITICAL DIRECTIVE]: Candidate has exhausted their knowledge on ${updatedTopics[activeIdx].name}. DO NOT ask any more questions about it. Formulate a natural transition and immediately ask a question about the NEXT topic: ${nextTopicName}.`;
                    } else {
                        directive = `\n\n[IMMEDIATE TOPIC SHIFT]: Previous topic ${updatedTopics[activeIdx].name} is COVERED. Bridge naturally but firmly to: ${nextTopicName}.`;
                    }
            ...
            // Ensure agent sees the latest turns/topics
            updateAgentInstructions();
...
Return ONLY JSON:
{
  "topics": [
    ...
    }, [addConductorLog, finishConductorLog, syncUsage, updateAgentInstructions]);
    ...
    // CALL LIFECYCLE
    // ─────────────────────────────────────────────────────────────────────────────
    // Add this helper to sync everything to the agent
    ...
    const startCall = useCallback(async () => {
        ...
        try {
            // 1. Get LiveKit Token
            const room = `interview_${makeId()}`;
            setRoomName(room);
            const tokenRes = await fetch(`/ai-interview/api/livekit-token?room=${room}`);
            ...
            // 3. Setup Listeners
            roomObj.on(RoomEvent.TranscriptionReceived, (segments: TranscriptionSegment[], participant?: Participant) => {
                const text = segments.map(s => s.text).join(' ');
                if (!text || !participant) return;

                const isAgent = !participant.isLocal;
                const role = isAgent ? 'ai' : 'user';
                const itemId = segments[0].id || makeId();

                setLogs(prev => {
                    const idx = prev.findIndex(l => l.id === itemId);
                    if (idx === -1) return [...prev, { id: itemId, role, text, pending: false }];
                    const copy = [...prev];
                    copy[idx] = { ...copy[idx], text, pending: false };
                    return copy;
                });
                ...
                if (isAgent) {
                    ...
                    setIsAriaSpeaking(true);
                    // Inside RoomEvent.TranscriptionReceived handler...
                } else {
                    lastUserTextRef.current = text;
                    setIsAriaSpeaking(false);
                    ...
                    if (phaseRef.current === 'warmup') {
                        if (!processedItemsRef.current.has(itemId)) {
                            ...
                            if (warmupTurnsRef.current >= WARMUP_HARD_LIMIT) {
                                setPhase('interview');
                                phaseRef.current = 'interview';
                                generateStrategy(); // ADD THIS LINE: Triggers the topic creation
                            }
                            syncInstructionsRef.current();
                        }
                    }
                    ...
                    if (phaseRef.current === 'wrapup') {
                        if (!processedItemsRef.current.has(itemId)) {
                            processedItemsRef.current.add(itemId);
                            wrapupTurnsRef.current += 1;
                            setWarmupTurns(wrapupTurnsRef.current);

                            if (wrapupTurnsRef.current >= WRAPUP_HARD_LIMIT) {
                                setPhase('closing');
                                phaseRef.current = 'closing';
                                injectBrief('[SYSTEM]: Wrapup complete. Give a warm, genuine farewell and close the interview.');
                            }
                            syncInstructionsRef.current();
                        }
                    }
                    ...
                    if (phaseRef.current === 'interview') {
                        // ... existing conductor logic ...

                        if (conductorDebounceRef.current) clearTimeout(conductorDebounceRef.current);
                        conductorDebounceRef.current = setTimeout(() => {
                            runConductor(lastAiTextRef.current, text);
                        }, CONDUCTOR_DEBOUNCE_MS);
                    }
                }
            });
            ...
            roomObj.on(RoomEvent.ActiveSpeakersChanged, (speakers: Participant[]) => {
                const agentSpeaking = speakers.some(p => p.identity.includes('agent') || p.identity.includes('aria'));
                setIsAriaSpeaking(agentSpeaking);
            });
            ...
            setPhase('warmup');
            phaseRef.current = 'warmup'; // Force immediate update for the first turns

            // Sync initial brief once agent joins
            // 1. Build and sync the full initial system prompt
            const initialBrief = buildLiveBrief({
                phase: 'warmup',
                candidateName: candidateNameRef.current,
                topics: [],
                scores: [],
            ...
            const initialPrompt = buildRTSystemPrompt({
                cvPersonal: cvPersonalRef.current,
                cvTechnical: cvTechnicalRef.current,
                jdText: jdTextRef.current,
                candidateName: candidateNameRef.current,
                personality: behaviorRef.current.mood,
                briefText: initialBrief,
            });
    ...
    }, [isCallActive, cvText, jdText, generateStrategy, addConductorLog, finishConductorLog, syncUsage, runConductor]);
  ...
  `;
    ...
    return (
        <LiveKitRoom
            room={lkRoom || undefined} // Pass the shared room instance here
        ...
        >
            ...
            <div className="live">
                ...
                <div className="right-panel">
                    ...
                    {/* DEBUG: LIVE PROMPT */}
                    {currentInstructions && (
                        <div style={{ marginTop: 'auto', borderTop: '1px solid var(--border)', padding: '12px 14px', background: 'rgba(248,113,113,.03)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                                <span style={{ fontSize: 9, fontWeight: 700, color: 'var(--red)', fontFamily: 'var(--mono)' }}>DEBUG: LIVE LLM PROMPT</span>
                                <button onClick={() => navigator.clipboard.writeText(currentInstructions)} style={{ background: 'none', border: 'none', color: 'var(--t3)', fontSize: 7, cursor: 'pointer' }}>COPY</button>
                            </div>
                            <pre style={{ fontSize: 8, fontFamily: 'var(--mono)', whiteSpace: 'pre-wrap', color: 'var(--t2)', maxHeight: '400px', overflowY: 'auto', margin: 0 }}>
                                {currentInstructions}
                            </pre>
                        </div>
                    )}
