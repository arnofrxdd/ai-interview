...
const SILENCE_CHECK_MS = 22000;      // ms before checking if candidate is silent
...
const FILLER_PHRASES = [
  ...
  'That\'s a good one, give me a sec...',
  'Mmm, let me think...',
  'Right, okay...',
...
- Keep your speaking turns SHORT — 1-2 sentences per turn maximum

VOICE RULES (critical):
- This is a voice call. No markdown, no bullet points, no numbered lists, no emojis.
- Speak in natural flowing sentences only.
- Simple greetings and transitions: answer directly without any tool call.
...
SILENCE: If candidate is silent, gently check in after they asked for time.
DO NOT say function names, tool names, or any technical terms aloud. Ever.`;
  ...
  const armSilenceTimer = useCallback(() => {
    ...
    silenceTimerRef.current = setTimeout(() => {
      if (!isEndingRef.current && dcRef.current?.readyState === 'open') {
        sendRtMessage({ type: 'response.create', response: { instructions: 'The candidate has been quiet for a while. Gently ask if they need a moment or want the question rephrased. Keep it to one short sentence.' } });
      }
    }, SILENCE_CHECK_MS);
  }, []);

  const disarmSilenceTimer = useCallback(() => {
  ...
  const submitToolResult = useCallback((
    ...
    const instruction = deliveryInstruction
      ?? `Deliver this as a natural continuation of the conversation — speak it as your own thought, not as if reading a result. Sound like a human interviewer who just thought it through. Result: "${result}"`;

    if (callId.startsWith('__client__')) {
      // Client-originated call — just send response.create
      sendRtMessage({ type: 'response.create', response: { instructions: instruction } });
    } else {
      // RT-originated tool call — submit output first, then trigger response
      sendRtMessage({
...
TASK: ${task}`.trim();
  ...
  }, [buildSystemPrompt, submitToolResult]);
  ...
  const handleRtEvent = useCallback((ev: Record<string, unknown>) => {
    switch (ev.type as string) {
      ...
      case 'response.function_call_arguments.done': {
        ...
        // Speak filler IMMEDIATELY when tool call is detected so the candidate hears something
        if (name === 'escalate_thinking') {
          // response.create → RT speaks the filler phrase aloud, then stops and waits
          sendRtMessage({
            type: 'response.create',
            response: {
              modalities: ['audio', 'text'],
              instructions: `Say EXACTLY this phrase and nothing else: "${randomFiller()}" — then stop speaking and wait silently for the tool result.`,
            },
          });
        }

        if (name === 'fetch_cv_info') {
          // response.create → RT speaks the CV filler phrase aloud, then stops and waits
          sendRtMessage({
            type: 'response.create',
            response: {
              modalities: ['audio', 'text'],
              instructions: `Say EXACTLY this phrase and nothing else: "${randomCvFiller()}" — then stop speaking and wait silently for the tool result.`,
            },
          });
        }
        ...
        switch (name) {
          ...
          case 'escalate_thinking': {
            ...
            if (escalStateRef.current === 'fetching') {
              ...
              sendRtMessage({
                type: 'response.create',
                response: { instructions: 'Continue the interview naturally. Ask the next question.' },
              });
              break;
            }
          ...
          case 'end_interview': {
            submitToolResult(
              callId,
              'Interview complete.',
              "Thank the candidate warmly for their time. Tell them the interview is complete and they'll hear back soon. Wish them well. Keep it to 2 sentences.",
            );
    ...
    addLog, updateLog, armSilenceTimer, disarmSilenceTimer,
    pruneRtContext, updateSmartNotes, handleEscalation,
    sendRtMessage, submitToolResult,
  ]);
      ...
      dc.onopen = () => {
        ...
        setTimeout(() => {
          ...
          sendRtMessage({
            type: 'response.create',
            response: {
              instructions: `Welcome the candidate ${candidateName ? `named ${candidateName}` : ''} warmly. Introduce yourself as Aria, a senior interviewer. Tell them this is an interview for the role and ask if they're ready to begin. Keep it to 2-3 sentences — natural, professional, friendly.`,
            },
          });
  ...
  }, [isCallActive, cvText, jdText, buildSystemPrompt, sendRtMessage, handleRtEvent, armSilenceTimer]);
